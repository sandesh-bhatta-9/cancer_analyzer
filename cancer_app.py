...
def get_compound_gene_targets():
    ...
    # This is a curated list of known gene targets, not a full "pathway".
    # This explains why the gene sets for compounds are smaller than for KEGG pathways.
    return {
        # Chemotherapy Drugs
...
st.sidebar.markdown("---")
# REMOVED: The checkbox to toggle views is removed to always show the detailed graph.
...
# --- Graph Building Logic (Now always detailed) ---
if analysis_mode != 'Cancer Pathway vs. Cancer Pathway':
        ...
        for c_name in selected_compounds:
            ...
            for gene in pathway_genes:
                if gene in shared_genes:
                    G.add_node(gene, type='gene', size=15, color='magenta') # Highlight shared genes
                else:
                    G.add_node(gene, type='unique_pathway_gene', size=8, color='#a6cee3') # Unique pathway genes are light blue
                G.add_edge(p_name, gene)
            
            # Add all compound genes and their edges
            for gene in compound_genes:
                G.add_edge(c_name, gene)
                if gene not in shared_genes: # Add node if not already added as a shared gene
                    G.add_node(gene, type='unique_compound_gene', size=8, color='#b2df8a') # Unique compound targets are light green
    ...
    for pathway_name, genes in pathway_genes_map.items():
        ...
        for gene in genes:
            if gene in shared_genes_set:
                G.add_node(gene, type='gene', size=15, color='magenta') # Shared genes are magenta
            else:
                # Unique genes get a color specific to their pathway
...
if G.number_of_nodes() > 0:
    ...
    st.info("The network shows all individual genes. Shared genes are highlighted (magenta), while unique genes are colored based on their pathway or compound.")
    ...
    for node_type, nodes in nodes_by_type.items():
        if nodes:
            ...
            trace_name = "Other"
            # Logic to create a distinct legend item for each pathway's unique genes
            if node_type.startswith('unique_gene_'):
                pathway_name = node_type.replace('unique_gene_', '')
                trace_name = f'Unique to {pathway_name}'
